...
data = pd.read_csv("mbt/mbti_df.csv") 
...
#@app.route('/')
def getmbt(search):    
    #user_input = str(input('Talk to me'))
    
#    user_input = ' '.join(str(request.args.get("search")).split('+'))
    user_input = str(search)	
    # The type is just a dummy so that the data prep fucntion can be reused
    mydata = pd.DataFrame(data={'type': ['INTJ'], 'posts': [user_input]})

    user_input, dummy  = pre_process_data(mydata, remove_stop_words=True)

    my_X_cnt = cntizer.transform(user_input)
    ...
    #if translate_back(result) == "INTJ" :
     #   print(pers_dict['I'] + pers_dict['N'] + pers_dict['T'] + pers_dict['J'])
    # print(translate_back(result))

    disp_res = []
    
    res_list = list(translate_back(result))
    for i in res_list:
        # print(pers_dict[i]+' ')
        disp_res.append(pers_dict[i])
    # print(disp_res)
    disp_res_string = " "
    return disp_res_string.join(disp_res)
